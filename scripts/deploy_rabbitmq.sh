...
if [[ $flagDestroy != "true" ]]; then
  ...
  terraform -chdir=../terraform/rabbitmq init \
    ...
    -backend-config="key=${NEWRELIC_ACCOUNT_ID}-rabbitmq-local"

  # Plan Terraform
  terraform -chdir=../terraform/rabbitmq plan \
  ...
  if [[ $flagDryRun != "true" ]]; then
    terraform -chdir=../terraform/rabbitmq apply tfplan
  fi
else

  # Destroy Terraform
  terraform -chdir=../terraform/rabbitmq destroy \
    -var NEW_RELIC_ACCOUNT_ID=$NEWRELIC_ACCOUNT_ID \
    -var NEW_RELIC_API_KEY=$NEWRELIC_API_KEY \
    -var NEW_RELIC_REGION=$NEWRELIC_REGION
fi
